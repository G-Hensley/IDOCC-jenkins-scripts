...
def print_scan_summary(detections_data):
    """Print a summary of scan results from the detections endpoint."""
    metadata = detections_data.get("metadata", {})
    total_active = metadata.get("totalActiveVulnerabilities", 0)
    total_high = metadata.get("totalHighSeverityVulnerabilities", 0)
    total_tests = metadata.get("totalTests", 0)
    total_resolved = metadata.get("totalVulnerabilitiesResolved", 0)

    print(f"\nScan Statistics:")
    print(f"  Total Tests Run:     {total_tests}")
    print(f"  Active Vulnerabilities: {total_active}")
    ...
    # Break down by category/test
    severity_counts = {}
    detections = detections_data.get("detections", [])
    for detection in detections:
        vuln_count = detection.get("data", {}).get("numActiveVulnerabilities", 0)
        if vuln_count > 0:
            test_name = detection.get("test", {}).get("name", "Unknown")
            vulns = detection.get("data", {}).get("vulnerabilities", [])
            for vuln in vulns:
                qualifier = vuln.get("testResult", {}).get("cvssQualifier", "Unknown")
                severity_counts[qualifier] = severity_counts.get(qualifier, 0) + 1

    print(f"\nVulnerability Summary: {total_active} active vulnerabilities")
    ...
    for detection in detections:
        vulns = detection.get("data", {}).get("vulnerabilities", [])
        for vuln in vulns:
            method = vuln.get("method", "?").upper()
            resource = vuln.get("resource", "?")
            score = vuln.get("testResult", {}).get("cvssScore", "?")
            qualifier = vuln.get("testResult", {}).get("cvssQualifier", "?")
            test_name = detection.get("test", {}).get("name", "Unknown")
            print(f"  [{qualifier} {score}] {method} {resource} - {test_name}")
...
def main():
    ...
    # Fetch detections — this is the source of truth for vulnerabilities
    print("\nFetching vulnerability detections...")
    detections_data = get_detections(token, app_id, inst_id)
    total_findings = print_scan_summary(detections_data)
    ...
    print("\n" + "=" * 60)
    if total_findings > 0:
        print(f"Scan COMPLETED - {total_findings} active vulnerabilities found")
        print("=" * 60)
        sys.exit(1)
    else:
        print("Scan COMPLETED - No active vulnerabilities found")
        print("=" * 60)
